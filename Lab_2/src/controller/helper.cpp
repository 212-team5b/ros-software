...
void RobotPose::update(float dPhiL, float dPhiR) {
    ...
    dTh = r*(dPhiR-dPhiL)/(2*b) ;
    dX = (r/2)*(dPhiR + dPhiL)*cos(Th);
    dY = (r/2)*(dPhiR + dPhiL)*sin(Th);
...
void PathPlanner::navigateTrajU(const RobotPose & robotPose) {
    ...
    // Quarter Circle (first point)
    if (robotPose.pathDistance < .125*PI) { 
        float robotVel = .2, K = 1/.25;
        updateDesiredV(robotVel, K);
    } 
    // Backwards Quarter circle (second point)
    else if (robotPose.pathDistance < .25*PI){
         float robotVel = -.2, K = -1/.25;
         updateDesiredV(robotVel,K);
    }
//    // Quarter Circle (third point)
//    else if(robotPose.pathDistance < PI*.325){
//         float robotVel = .2, K = 0;
//         updateDesiredV(robotVel,K);
//    }

    // Straight (fourth point)
    else if(robotPose.pathDistance < PI*.5){
         float robotVel = .2, K = 0;
         updateDesiredV(robotVel,K);
    }
...
void PathPlanner::updateDesiredV(float robotVel, float K) {
    ...
    // MODIFY CODE BELOW TO SET THE CORRECT VALUES
    desiredWV_R = (b*K + 1)*robotVel;
    desiredWV_L = (1- b*K)*robotVel;
}
